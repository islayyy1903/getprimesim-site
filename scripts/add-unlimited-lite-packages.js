...
data.forEach(country => {
  if (country.standardPackages && country.standardPackages.length > 0) {
    ...
    if (code) {
      ...
      // Create unlimited lite packages for each validity
      country.unlimitedLitePackages = allValidities.map((validity, index) => {
        const days = parseInt(validity.split(' ')[0]);
        let multiplier;
        let isPopular = false;
        
        if (days === 7) multiplier = 1.5;
        else if (days === 14) multiplier = 2.0;
        else if (days === 15) {
          multiplier = 2.5;
          isPopular = true;
        }
        else if (days === 21) multiplier = 3.0;
        else if (days === 30) multiplier = 4.0;
        else if (days === 60) multiplier = 7.0;
        else if (days === 90) multiplier = 10.0;
        else multiplier = 1.5 + (days / 7) * 0.5; // Dynamic calculation for other durations
        
        return {
          name: `${country.name} – Unlimited Lite ${validity}`,
          bundleId: `esim_unlimited_lite_${days}D_${code}_V2`,
          data: "Unlimited Lite",
          validity: validity,
          ...
          shortDescription: "High-speed data with fair usage"
        };
      });
...
console.log(`Added Unlimited Lite packages to ${added} countries`);
